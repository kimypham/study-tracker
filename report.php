<?php
// Study Tracker - Report (view statistics) page, version 1.3, Kim Pham
include 'login_check.php';
unset($_SESSION['error']);
include 'display.php';
?>
<html>
<head>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="stylesheet" type="text/css" href="./CSS/reset.css">
	<link rel="stylesheet" type="text/css" href="./CSS/style.css">
	<link rel="stylesheet" type="text/css" href="./CSS/other-style.css">
	<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
</head>
<header>
	<div class="header">
		<a href="index.php" title="Home" class="logo"><img src="./images/logo-light.png" width="250px"></a>
		<div class="header-right">
			<a href="index.php" class="menu">HOME</a>
			<a>VIEW STATISTICS</a>
			<a href="help.php" class="menu">HELP</a>
			<a href="settings.php" class="menu">SETTINGS</a>
			<a href="logout.php" class="menu">LOG OUT</a>
		</div>
	</div>
</header>

<body class="scrollbar">
<div class="main" style="width:35em;">
	<p class="title"> View statistics </p>
	<br>
	<a href="report_before.php" class="no_link"><button class="btn">Back one day</button></a>
	<a href="report_after.php" class="no_link"><button class="btn disabled" disabled>Forward one day</button></a>
	<a href="report.php" class="no_link"><button class="btn disabled" disabled>Today</button></a>
	<form action="report.php" method="POST">
		<div class="record_caption">Search for a record:</div>
		<input class="record_search" type="date" name="date" max="<?php echo $_SESSION['realDate']; ?>">
		<input class="hidden" type="submit" name="dateSearch">
	</form>
	<br><br>
	<div style="text-align:justify;">
	<?php
		// displays all of the user's subjects/tasks and sorts them if neccessary (from display.php)
		if (isset($_GET['sort'])) {	// changes sorting only if changed
			$_SESSION['sort'] = $_GET['sort'];
			$user->Display($_SESSION['sort']);
		} elseif (isset($_SESSION['sort'])) {	// uses previous sorting preference
			$user->Display($_SESSION['sort']);
		} else {
			$user->Display('original');
		}
		
		if (isset($_SESSION['error_list'])) {echo "<p>".$_SESSION['error_list']."</p>";}
		elseif (isset($_SESSION['error'])) {echo "<p>".$_SESSION['error']."</p>";}
	?>
	</div>
<div class="button_container">
		<a href="index.php" class="no_link"><button class="btn btn_right">Go back</button></a>
	</div>
</div>
<div id="piechart" style="float:right; margin-top:1.5em;"></div>
</body>
</html>