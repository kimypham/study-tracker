...
if (isset($_POST['dateSearch'])) {
	if ($_SESSION['realDate'] == $_POST['date']){
		$_SESSION['num'] = 0;
	}else{
		$date1=date_create($_SESSION['realDate']);
		$date2=date_create($_POST['date']);
		$diff=date_diff($date1,$date2);
		$_SESSION['num'] = $diff->format("%a")-1;
		header('Location: report_before.php');
		
	}
	$_SESSION['todayDate'] = $_POST['date'];
}
?>