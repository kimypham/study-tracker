...
public function Display($sort) {
		...
		if ($numrows!=0) {
			$list = array();
				...
				if ($sort == "a-z") {
					ksort($list, SORT_STRING | SORT_FLAG_CASE);	// sort by key (subject/task name), alphabetical order (A-Z), case insensitive (sorted regardless of uppercase and lowercase letters)
				} 
				elseif ($sort == "z-a") {
					krsort($list, SORT_STRING | SORT_FLAG_CASE); // sort by key (subject/task name), alphabetical order (Z-A), case insensitive
				}
				elseif ($sort == "low-high") {
					asort($list);	// sort by value (time)
				}
				elseif ($sort == "high-low") {
					arsort($list);	// sort by value (time)
				}
	?>
	<!-- select dropdown -->
