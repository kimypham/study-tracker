<?php
// Study Tracker - write to XML from settings and read from XML files functions, version 1.1, Kim Pham
...
class Settings {
	...
	public function Update($timerType, $timerLength, $timerRing, $timerNotif) {
	...
	public function Auto(){
		...
		foreach ($xml->input as $input) {
			$_SESSION['type'] = (string)$input->type;
			$_SESSION['length'] = (string)$input->length;
			$_SESSION['ring'] = (string)$input->ring;
			$_SESSION['notif'] = (string)$input->notif;
		}
		if ($_SESSION['length'] == 00) {	// sets default timer length
			$_SESSION['minutes'] = 30;
		}
		if ($_SESSION['length']>=60) {	// changes user input to hours and minutes format if necessary
			$_SESSION['hours'] = floor($_SESSION['length']/60);	// converts minutes to hours and only gets the whole number part (no of hours)
			$_SESSION['minutes'] = ($_SESSION['length']/60 - $_SESSION['hours']) * 60; 	// converts minutes to hours and only gets the decimal places before converting back to minutes (no of minutes)
		} else {
			$_SESSION['minutes'] = $_SESSION['length'];
			$_SESSION['hours'] = 0;
		}
...
if (isset($_POST['submit'])) {	// gets all variables from timer form
	...
	$user->Update($timerType, $timerLength, $timerRing, $timerNotif);	
}
?>